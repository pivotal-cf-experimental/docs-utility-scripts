...
function doreplacement(text)
{
	...
	text = text.replace(/(`.*_.*` errand)/gm,'$1 <%# Errands and placeholders use hyphens, not underscores. %>');
	text = text.replace(/(`.*_.*` errands)/gm,'$1 <%# Errands and placeholders use hyphens, not underscores. %>');
	...
	text = text.replace(/(^\s*#{2,3}[\s\w](?!.*\<a id)\s*(?!(Features|Security Fixes|Resolved Issues|Known Issues|Compatibility)).*$)/gm,'$1 <%# Make H2 and H3 headers anchors unless the ToC is already over-sized. %>');
	...
	text = text.replace(/(<td[^\>]*>\s*none\s*\<\/td\>(?![^£]*\<%# ECS %\>))/gm,'$1<%# Use n/a, in italics, to mark an empty table cell. %>');
	text = text.replace(/(\sorg\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# "organization" is preferred unless referring to the Cloud Foundry org concept. %>');
	...
	text = text.replace(/(\bplatform[^\.]* Pivotal Platform\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use "platform" in within a sentence or two of the brand name. %>');
	...
	text = text.replace(/(\bWe recommend\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Specify the party that recommends (Pivotal, Cloud Foundry, etc). %>');
	text = text.replace(/(\bwe'd\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bWe'd\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bwe'll\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bWe'll\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bwe're\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bWe're\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bwe've\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bWe've\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Do not use contractions. %>');
