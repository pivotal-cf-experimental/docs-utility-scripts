...
function doreplacement(text)
{
	...
	text = text.replace(/((```|\<\/code\>|\<\/pre\>)\n{2,}\s*Where\b)/gm,'$1<%# Place |Where| immediately below the closing code tag. %>');
	text = text.replace(/(^(?!\n)\d\.[^:]*\n\n(?=\s+\d\.))/gm,'$1<%# Place a colon at the end of the step that precedes the nested steps. %>');
	text = text.replace(/(^(?!\n)\s{3,}\d\.)/gm,'$1<%# Place this step at the start of the line or, if nested, indent it 2 spaces. %>');
	text = text.replace(/(^(?!\n)\s{4}(```|<\/code>|<\/pre>)\n(?!\n)(\s{0,3}|\s{5,})[^\s])/gm,'$1<%# Indent this line the same number of spaces as the closing code tag immediately above. %>');
	text = text.replace(/(^(?!\n)\s{4}[^\s]+\n(\s{0,3}|\s{5,})(\*|\+))/gm,'$1<%# Indent this bullet the same amount as the line above. %>');
	text = text.replace(/(^(?!\n)\s{8}(```|<\/code>|<\/pre>)\n(?!\n)(\s{0,7}|\s{9,})[^\s])/gm,'$1<%# Indent this line the same number of spaces as the closing code tag immediately above. %>');
	text = text.replace(/(^(?!\n)\s{8}[^\s]+\n(\s{0,7}|\s{9,})(\*|\+))/gm,'$1<%# Indent this bullet the same amount as the line above. %>');
	text = text.replace(/(^(?!\n)\s+\d\.[^:]*:\n\n(\s{0,7}|\s{9,})(?=(```|<pre|<code)))/gm,'$1<%# Indent 8 spaces. %>');
	text = text.replace(/(^\d\.[^:\n]*:\n\n(\s{0,1}|\s{3,})(?=(\d\.)))/gm,'$1<%# If a nested step, indent it 2 spaces. %>');
	text = text.replace(/(^\d\.[^:\n]*:\n\n(\s{0,3}|\s{5,})(?=(```|<pre|<code)))/gm,'$1<%# Indent 4 spaces. %>');
	...
	text = text.replace(/(\(https:..community.pivotal.io.*\)(?!.*\n?.*VMware Tanzu Support knowledge base).*$)/gm,'$1<%# Type |in the VMware Tanzu Support knowledge base| somewhere in the cross-reference sentence. %>');
	...
	text = text.replace(/(\bhtml#[a-z-]*\) above\b)/gm,'$1<%# Delete |above|: the reference is in another topic. %>');
	text = text.replace(/(\bhtml#[a-z-]*\) below\b)/gm,'$1<%# Delete |below|: the reference is in another topic. %>');
	...
	text = text.replace(/(\<a id=[^\d\>]*\d\d[^\>]*\>)/gm,'$1<%# Place dashes between digits in release notes anchor IDs. %>');
	text = text.replace(/(\<a id=[^\>]*\.)/gm,'$1<%# Do not use periods in anchors. %>');
	text = text.replace(/(##+[^\n]*\>\s*[A-Z][a-z-]+ing\W\b)/gm,'$1<%# Procedure headers use imperatives, not gerunds. %>');
	text = text.replace(/(##+\s*\d\.)/gm,'$1<%# Do not format a numbered step as a header. %>');
	text = text.replace(/(^##\s*[A-Z](?!(eatures|ecurity Fix|esolved Issue|nown Issue|ompatibility|ackages)).*$)/gm,'$1<%# Make this header an anchor unless the subnav is unusually long. %>');
	text = text.replace(/(^###\s*[A-Z](?!(eatures|ecurity Fix|esolved Issue|nown Issue|ompatibility|ackages)).*$)/gm,'$1<%# Make this header an anchor unless the subnav is unusually long. %>');
	text = text.replace(/(```[^`]*[A-Z][A-Z]+[^`]*```(?![\n\s]*Where))/gm,'$1<%# Define any non-obvious placeholders present in the code snippet in the style of |Where PLACEHOLDER is...| %>');
	text = text.replace(/(`pdc` CLI tool\b)/gm,'$1<%# |<%= vars.product_abbr %> CLI tool| is preferred. %>');
	text = text.replace(/(`pdc` cli tool\b)/gm,'$1<%# |<%= vars.product_abbr %> CLI tool| is preferred. %>');
	text = text.replace(/(`pdc` CLI Tool\b)/gm,'$1<%# |<%= vars.product_abbr %> CLI tool| is preferred. %>');
	text = text.replace(/(`.*[_\A-Z].*` errand\b)/gm,'$1<%# Errands tend to be lowercase and they use dashes instead of underscores or spaces. %>');
	text = text.replace(/(`.*[_\A-Z].*` errands\b)/gm,'$1<%# Errands tend to be lowercase and they use dashes instead of underscores or spaces. %>');
	text = text.replace(/(:alt =\> .[^\{\)]*\<%= vars\.[^\)]*\) %\>)/gm,'$1<%# Escape ERB variables within ERB tags with #{}, as in #{vars.product_short}. %>');
	...
	text = text.replace(/(\bdocs-pcf-staging.cfapps.io.*$)/gm,'$1<%# Do not link to staging versions of the docs. %>');
	...
	text = text.replace(/(\bHOST\b)/gm,'$1<%# |BOSH-DIRECTOR-IP| is preferred. %>');
	text = text.replace(/(\bPATH-TO-MANIFEST[^\.])/gm,'$1<%# Include the file extension, as in |PATH-TO-MANIFEST.yml|. %>');
	text = text.replace(/(\bPATH-TO-VARIABLES[^\.])/gm,'$1<%# Include the file extension, as in |PATH-TO-VARIABLES.yml|. %>');
	text = text.replace(/(\bPRIVATE-KEY-FILE\b)/gm,'$1<%# |PATH-TO-PRIVATE-KEY| is preferred. %>');
	text = text.replace(/(\bSERVICE-INSTANCE(?!-NAME))/gm,'$1<%# |SERVICE-INSTANCE-NAME| is preferred. %>');
	text = text.replace(/(\bVERSION(?!-NUMBER))/gm,'$1<%# |VERSION-NUMBER| is preferred. %>');
	...
	text = text.replace(/(\brun:[\n\s]*\<code\>[\s\n\~]*\$)/gm,'$1<%# We only include dollar signs in terminal snippet examples. %>');
	text = text.replace(/(\brun[^:]*:[\n\s]*\<pre class\b)/gm,'$1<%# Commands are formatted in triple backtick blocks, not terminal snippets. %>');
	...
	text = text.replace(/(\bversion [\d\.]+)/gm,'$1<%# |v#| is the preferred style. %>');
	text = text.replace(/(\bVersion [\d\.]+)/gm,'$1<%# |v#| is the preferred style. %>');
	text = text.replace(/(\bversions [\d\.]+)/gm,'$1<%# |v#| is the preferred style. %>');
	text = text.replace(/(\bVersions [\d\.]+)/gm,'$1<%# |v#| is the preferred style. %>');
	text = text.replace(/(\bVersions \<\= [\d\.]+)/gm,'$1<%# |v#| is the preferred style. %>');
	text = text.replace(/(\bVersions \>\= [\d\.]+)/gm,'$1<%# |v#| is the preferred style. %>');
	text = text.replace(/(\bversions \<\= [\d\.]+)/gm,'$1<%# |v# or earlier| is the preferred style. %>');
	text = text.replace(/(\bversions \>\= [\d\.]+)/gm,'$1<%# |v# or earlier| is the preferred style. %>');
	text = text.replace(/(---\n\n[^#]*(?=(#[^#]|###)))/gm,'$1<%# The first header must be ## -- no smaller, no larger. %>');
	...
	// Remove delimiters for HTML formatting tags other than code and bold

	text = text.replace(/(\<%# ¥SHS %\>)/gm,'');
	text = text.replace(/(\<%# EHS %\>)/gm,' ');

	// Add delimiters for code tags and strong tags
	...
	text = text.replace(/(\<%= image_tag[^\s]*)/gm,'<%# £SCS %>$1<%# ECS %>');
	text = text.replace(/(\<%= partial[^\{%]* %\>)/gm,'<%# £SCS %>$1<%# ECS %>');
	text = text.replace(/(\<a href=[^\>]*\>)/gm,'<%# £SCS %>$1');
	text = text.replace(/(\<a id=[^\>]*\>)/gm,'<%# £SCS %>$1');
	text = text.replace(/(\<a name=[^\>]*\>)/gm,'<%# £SCS %>$1');
	text = text.replace(/(\<b\>)/gm,'<%# £SCS %>$1');
	text = text.replace(/(\<code\>)/gm,'<%# £SCS %>$1');
	text = text.replace(/(\<img src=[^\s]*)/gm,'<%# £SCS %>$1<%# ECS %>');
	text = text.replace(/(\<pre[^\>]*\>)/gm,'<%# £SCS %>$1');
	text = text.replace(/(\<strong\>)/gm,'<%# £SCS %>$1');
	text = text.replace(/(`+[^`]*`+)/gm,'<%# £SCS %>$1<%# ECS %>');
	text = text.replace(/(^---\n[^\n]*\n[^\n]*\n---)/gm,'<%# £SCS %>$1<%# ECS %>');

	// Comment on style errors that have nothing to do with code

	text = text.replace(/(\(Go\) Router\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Gorouter| is preferred. %>');
	text = text.replace(/(2012R2(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The short product name is #{vars.windows_runtime_abbr} 2012 R2. %>');
	text = text.replace(/(\bapp runtime(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_full} %>');
	text = text.replace(/([^a][^l][\n\s]Compliance Scanner(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]Concourse(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bDev Connect(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bdevconnect(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bDevconnect(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bDevConnect(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bdevconnect(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bDevconnect(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]Developer Console(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bElastic Runtime(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_full} %>');
	text = text.replace(/(\bERT(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_full} %>');
	text = text.replace(/([^a][^l][\n\s]Event Alerts(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]FIM(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_abbr} %>');
	text = text.replace(/([^a][^l][\n\s]GemFire(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]Greenplum(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]Healthwatch(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]IPsec(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bIPSec for PCF(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/([^a][^l][\n\s]Isolation Segment(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bMySQL for Pivotal Platform(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bODB(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{product_abbr} %>');
	text = text.replace(/(\bon demand broker(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn demand broker(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn Demand Broker(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn Demand broker(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/([^a][^l][\n\s]On Demand Services SDK(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]on demand services SDK(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bOn-Demand Broker(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn-demand Broker(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/([^a][^l][\n\s]On-Demand Services SDK(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]on-demand services SDK(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bOperations Manager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager} %>');
	text = text.replace(/(\bOps Man\b(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager} %>');
	text = text.replace(/([^a][^l][\n\s]Ops Manager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager} %>');
	text = text.replace(/(\bops manager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager} %>');
	text = text.replace(/(\bOpsMan\b(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager} %>');
	text = text.replace(/(\bOpsManager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager} %>');
	text = text.replace(/(\bopsmanager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager} %>');
	text = text.replace(/(\bPAS(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_abbr} %>');
	text = text.replace(/(\bPBS(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bPDC(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_abbr} %>');
	text = text.replace(/(\bPFS(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bPivNet(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bPivotal Anti-Virus(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal App Service(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_full} %>');
	text = text.replace(/(\bPivotal app service(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_full} %>');
	text = text.replace(/(\bPivotal Application Service(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_full} %>');
	text = text.replace(/(\bPivotal application service(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_full} %>');
	text = text.replace(/(\bPivotal Backup and Restore(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Compliance Scanner(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Concourse(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Container Service(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.k8s_runtime_full} %>');
	text = text.replace(/(\bPivotal CredHub Service Broker(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Developer Console(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Event Alerts(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal File Integrity Monitoring(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal GemFire(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_name} %>');
	text = text.replace(/(\bPivotal Greenplum(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Healthwatch(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal IPsec(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Isolation Segment(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal On Demand Services SDK(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Ops Manager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.ops_manager_full} %>');
	text = text.replace(/(\bPivotal Postgre[Ss](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Push Notification(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal RabbitMQ(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Service Instance Manager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPivotal Single Sign-On(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bPKS(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Unless this is part of a VMware or Dell Tech brand name, use #{vars.k8s_runtime_full} %>');
	text = text.replace(/([^a][^l][\n\s]Postgre[Ss](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]Push Notification(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bRabbitMQ \[Kubernetes\](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bRabbitMQ &#91;Kubernetes&#93;(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bRabbitMQ for Kubernetes(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bRabbitMQ for Pivotal Platform(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bRabbitMQ for VMware Tanzu \[Kubernetes\](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bRabbitMQ for VMware Tanzu &#91;Kubernetes&#93;(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bRedis for Pivotal Platform(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/([^a][^l][\n\s]Service Instance Manager(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/([^a][^l][\n\s]Single Sign-On(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_short} %>');
	text = text.replace(/(\bsingle sign-on(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/([^\>] Installation Dashboard(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# #{vars.ops_manager} Installation Dashboard| is preferred. %>');
	text = text.replace(/([^\>] installation dashboard(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# #{vars.ops_manager} Installation Dashboard| is preferred. %>');
	text = text.replace(/(\bTAS(?! for)(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Depending on the product, use #{vars.app_runtime_abbr} or #{vars.windows_runtime_abbr}. %>');
	text = text.replace(/(\bTAS for Kubernetes(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_k8s_abbr} %>');
	text = text.replace(/(\bTAS for VMs \[Windows\](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{windows_runtime_abbr} %>');
	text = text.replace(/(\bTAS for VMs &#91;Windows&#93;(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{windows_runtime_abbr} %>');
	text = text.replace(/(\bTAS for VMs(?=([\.\n]| [^\[&]))(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.app_runtime_abbr} %>');
	text = text.replace(/(\bPivotal Network(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMT network(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMT Network(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMTN(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMware Network(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMware network(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMware Tanzu Network(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMware Tanzu network(?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_network} %>');
	text = text.replace(/(\bVMware Tanzu RabbitMQ \[K8s\](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bVMware Tanzu RabbitMQ \[VMs\](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bVMware Tanzu SQL \[MySQL\](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	text = text.replace(/(\bVMware Tanzu SQL \[VMs\](?=[^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use #{vars.product_full} %>');
	...
	text = text.replace(/(2012R2(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The short product name is |<%= vars.windows_runtime_abbr %> 2012 R2|. %>');
	text = text.replace(/(\b[a-z]\.\s[A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Write all ordered steps as numbers. %>');
	...
	text = text.replace(/(\bAvailability zones\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |AZ| after the first use. %>');
	text = text.replace(/(\bbe [a-z-]*ed by\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Active voice is preferred. %>');
	text = text.replace(/(\s\d{1,2}.\d{1,2}\.\d{1,2}(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Version numbers are written as |v1.11.11| %>');
	...
	text = text.replace(/(\bCertificate signing requests\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |CSR| after first use. %>');
	text = text.replace(/(\bCertificate Signing Requests\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |CSR| after first use. %>');
	text = text.replace(/([\n\s]check (?!mark)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |verify|, |ensure|, and |confirm| are all preferred. %>');
	text = text.replace(/(\bclick [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements. %>');
	text = text.replace(/(\bClick [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements. %>');
	text = text.replace(/(\bclick the [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements. %>');
	text = text.replace(/(\bClick the [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements. %>');
	...
	text = text.replace(/(\bCommand line interface\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |CLI| after first use. %>');
	text = text.replace(/(\bcommand line interface\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |CLI| after first use. %>');
	text = text.replace(/(\bcomplete [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |complete|. %>');
	text = text.replace(/(\bComplete [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Complete|. %>');
	text = text.replace(/(\bcomplete [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |complete|. %>');
	text = text.replace(/(\bComplete [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Complete|. %>');
	text = text.replace(/(\bcomplete [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |complete|. %>');
	text = text.replace(/(\bComplete [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Complete|. %>');
	text = text.replace(/([^a][^l][\n\s]Compliance Scanner(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([^a][^l][\n\s]Concourse(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	...
	text = text.replace(/(\bDevConnect(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([^a][^l][\n\s]Developer Console(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bdo [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |do|. %>');
	text = text.replace(/(\bDo [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Do|. %>');
	text = text.replace(/(\bdo [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |do|. %>');
	text = text.replace(/(\bDo [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Do|. %>');
	text = text.replace(/(\bdo [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |do|. %>');
	text = text.replace(/(\bDo [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Do|. %>');
	text = text.replace(/(\se\.g:\s\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |For example| is preferred. %>');
	text = text.replace(/(\se\.g\.\s\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |For example| is preferred. %>');
	text = text.replace(/(\se\.g\.,\s\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |For example| is preferred. %>');
	text = text.replace(/(\se\.g\.:\s\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |For example| is preferred. %>');
	text = text.replace(/(\seg\.\s\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |For example| is preferred. %>');
	text = text.replace(/(\seg\.:\s\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |For example| is preferred. %>');
	text = text.replace(/(\bElastic Runtime(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.app_runtime_full %>');
	text = text.replace(/(\bERT(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.app_runtime_full %>');
	text = text.replace(/([^a][^l][\n\s]Event Alerts(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bexecute [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |execute|. %>');
	text = text.replace(/(\bExecute [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Execute|. %>');
	text = text.replace(/(\bexecute [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |execute|. %>');
	text = text.replace(/(\bExecute [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Execute|. %>');
	text = text.replace(/(\bexecute [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |execute|. %>');
	text = text.replace(/(\bExecute [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Execute|. %>');
	text = text.replace(/([^a][^l][\n\s]FIM(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_abbr %>');
	...
	text = text.replace(/([^a][^l][\n\s]Greenplum(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([^a][^l][\n\s]Healthwatch(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\s\.html\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |HTML| is preferred. %>');
	text = text.replace(/(\s\.zip\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |ZIP| is preferred. %>');
	...
	text = text.replace(/(\bIndependent service vendors\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |ISV| after the first use. %>');
	text = text.replace(/(\bIndependent Service Vendors\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |ISV| after the first use. %>');
	text = text.replace(/(\bIP[\s\n](?![Aa]ddress)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not omit |address| from |IP address|. %>');
	text = text.replace(/(\bIPSec for PCF(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/([^a][^l][\n\s]IPsec(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bis [a-z-]*ed by\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Active voice is preferred. %>');
	text = text.replace(/([^a][^l][\n\s]Isolation Segment(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bLightweight Directory Access Protocol(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |LDAP| is preferred. %>');
	text = text.replace(/(\bmembers of a [a-z]*(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |x members| is more succinct than |members of x|. %>');
	text = text.replace(/([a-z]* or [a-z]* not\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	text = text.replace(/(\bMySQL for Pivotal Platform(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bnavigate to the [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements. %>');
	text = text.replace(/(\bNavigate to the [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements. %>');
	text = text.replace(/(^Note\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Put notes in Note box formatting per the wiki. %>');
	text = text.replace(/(\bnow [a-z-]*ed by\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Active voice is preferred. %>');
	text = text.replace(/([^a][^l][\n\s]On Demand Services SDK(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([^a][^l][\n\s]on demand services SDK(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([^a][^l][\n\s]On-Demand Services SDK(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([^a][^l][\n\s]on-demand services SDK(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	...
	text = text.replace(/(\bPAS(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.app_runtime_abbr %>');
	text = text.replace(/(\bPBS(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/((^|[^r] )PCF(?! (Dev|Development|Solutions))(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.platform_old %>');
	text = text.replace(/([\s\.]\d+\-\d+(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use &ndash; for a range, not a regular dash. %>');
	text = text.replace(/(\bperform [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |perform|. %>');
	text = text.replace(/(\bPerform [\w ]* procedure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Perform|. %>');
	text = text.replace(/(\bperform [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |perform|. %>');
	text = text.replace(/(\bPerform [\w ]* procedures\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Perform|. %>');
	text = text.replace(/(\bperform [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follow| is preferred over |perform|. %>');
	text = text.replace(/(\bPerform [\w ]* steps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Follow| is preferred over |Perform|. %>');
	text = text.replace(/(\bPFS(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bPivotal Anti-Virus(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal App Service(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.app_runtime_full %>');
	...
	text = text.replace(/(\bPivotal application service(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.app_runtime_full %>');
	text = text.replace(/(\bPivotal Backup and Restore(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/((^|[^r] )Pivotal Cloud Foundry(?! (Dev|Development|Solutions))(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.platform_old %>');
	text = text.replace(/(\bPivotal Compliance Scanner(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal Concourse(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal CredHub Service Broker(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal Developer Console(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	...
	text = text.replace(/(\bPivotal On Demand Services SDK(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal Ops Manager(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.ops_manager_full %>');
	text = text.replace(/((^|[^r] )Pivotal Platform\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.platform_name %>');
	text = text.replace(/(\bPivotal Postgre[Ss](?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal Push Notification(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal RabbitMQ(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal Service Instance Manager(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bPivotal Single Sign-On(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bplease(?! email)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use unless asking the reader to do you a favor, such as giving feedback. %>');
	text = text.replace(/(\bPlease(?! email)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use unless asking the reader to do you a favor, such as giving feedback. %>');
	text = text.replace(/([^a][^l][\n\s]Postgre[Ss](?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([^a][^l][\n\s]Push Notification(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bRabbitMQ for Kubernetes(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	...
	text = text.replace(/(\bRedis for Pivotal Platform(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/(\bselect [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements that you instruct the reader to interact with. %>');
	text = text.replace(/(\bSelect [A-Z](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Embolden the names of UI elements that you instruct the reader to interact with. %>');
	text = text.replace(/([^a][^l][\n\s]Service Instance Manager(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/([A-Za-z][a-z]+teen\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	text = text.replace(/([^a][^l][\n\s]Single Sign-On(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_short %>');
	text = text.replace(/(\bsingle sign-on(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_full %>');
	text = text.replace(/([^\>] Installation Dashboard(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |<%= vars.ops_manager %> Installation Dashboard| is preferred. %>');
	text = text.replace(/([^\>] installation dashboard(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |<%= vars.ops_manager %> Installation Dashboard| is preferred. %>');
	text = text.replace(/(\bSpring \d\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Spring Framework #| is preferred. %>');
	text = text.replace(/(\bTAS for VMs \[Windows\](?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.windows_runtime_abbr %>');
	text = text.replace(/(\bTAS for VMs &#91;Windows&#93;(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.windows_runtime_abbr %>');
	text = text.replace(/(\bTAS for VMs(?=([\.\n]| [^\[&]))(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.app_runtime_abbr %>');
	text = text.replace(/(\bTAS(?! for)(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Depending on the product, use vars.app_runtime_abbr or vars.windows_runtime_abbr %>');
	text = text.replace(/(\ba deploy\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |a deployment| is preferred. %>');
	text = text.replace(/(\bA deploy\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |A deployment| is preferred. %>');
	text = text.replace(/(\ba given\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	text = text.replace(/(\ba previous step\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Write |earlier in this procedure| or, if referring to a separate procedure, link to it. %>');
	...
	text = text.replace(/(\badministrate\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |administer| is preferred or, ideally, a plainer synonym such as |manage|. %>');
	text = text.replace(/(\bAdministrate\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Administer| is preferred or, ideally, a plainer synonym such as |Manage|. %>');
	text = text.replace(/(\badministrated\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |administered| is preferred or, ideally, a plainer synonym such as |managed|. %>');
	text = text.replace(/(\badministrates\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |administers| is preferred or, ideally, a plainer synonym such as |manages|. %>');
	text = text.replace(/(\badministrating\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |administering| is preferred or, ideally, a plainer synonym such as |managing|. %>');
	text = text.replace(/(\bAdministrating\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Administering| is preferred or, ideally, a plainer synonym such as |Managing|. %>');
	...
	text = text.replace(/(\bafter you are satisfied\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	text = text.replace(/(\bAfter you are satisfied\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	...
	text = text.replace(/(\bAn UAA\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |A UAA| is preferred. %>');
	text = text.replace(/(\band\/or\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |or| is preferred. %>');
	...
	text = text.replace(/(\bApr\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |April| is preferred. %>');
	...
	text = text.replace(/(\bAWS S3(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Amazon S3| is preferred. %>');
	text = text.replace(/(\bback-up\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The noun is |backup| and the verb is |back up|. %>');
	text = text.replace(/(\bBack-up\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The noun is |backup| and the verb is |back up|. %>');
	text = text.replace(/(\bbe sure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? |ensure| or even |before| might be better. %>');
	text = text.replace(/(\bBe sure\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? |Ensure| or even |Before| might be better. %>');
	...
	text = text.replace(/(\bBumps\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Updates| is preferred. %>');
	text = text.replace(/([^bo] button(?![^\[\(]*[\]\)])(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Delete |button| if redundant. For example, write |click Save| instead of |Click the Save button|. %>');
	text = text.replace(/(\bcall[\n\s]*\<(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |run| is preferred. %>');
	text = text.replace(/(\bcall:(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |run| is preferred. %>');
	text = text.replace(/(\bcalling[\n\s]*\<(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |running| is preferred. %>');
	text = text.replace(/(\bcalling:(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |running| is preferred. %>');
	text = text.replace(/(\bcan be\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider switching to active voice. %>');
	text = text.replace(/(\bcan't\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bcauses downtime of the\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |causes downtime for the| is preferred. %>');
	text = text.replace(/(\bcauses downtime on the\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |causes downtime for the| is preferred. %>');
	text = text.replace(/(\bcauses downtime to the\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |causes downtime for the| is preferred. %>');
	...
	text = text.replace(/(\bCheck\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Verify|, |Ensure|, and |Confirm| are all preferred. %>');
	text = text.replace(/(\bcheckmark\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |check mark| is preferred. %>');
	text = text.replace(/(\bchooses to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider if |choose| is redundant. %>');
	text = text.replace(/(\bclean-up\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The verb is |clean up|. The noun and adjective are both |cleanup|. %>');
	text = text.replace(/(\bclear text\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Unencrypted text is |plaintext|. Unformatted text is |plain text|. %>');
	text = text.replace(/(\bClear text\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Unencrypted text is |plaintext|. Unformatted text is |plain text|. %>');
	...
	text = text.replace(/(\bCloud Foundries\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Cloud Foundry| has no plural. |Cloud Foundry deployments| is likely what is meant. %>');
	text = text.replace(/(\bcloud foundries\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Cloud Foundry| has no plural. |Cloud Foundry deployments| is likely what is meant. %>');
	text = text.replace(/(\bCloud foundries\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Cloud Foundry| has no plural. |Cloud Foundry deployments| is likely what is meant. %>');
	text = text.replace(/(\bCloud Foundry credentials\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Depending on the product, use |vars.app_runtime_abbr credentials| or |vars.windows_runtime_abbr credentials| %>');
	...
	text = text.replace(/(\bCloud Foundry username and password\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Depending on the product, use |vars.app_runtime_abbr credentials| or |vars.windows_runtime_abbr credentials| %>');
	...
	text = text.replace(/(\bCloud Native-buildpack\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Cloud Native Buildpack| is preferred. %>');
	text = text.replace(/(\bCloud native-Buildpack\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Cloud Native Buildpack| is preferred. %>');
	text = text.replace(/(\bcloud native(?!(-|\s)(buildpack|Buildpack|application bundle|Applicable Bundle))(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |cloud-native| is preferred. %>');
	...
	text = text.replace(/(\bCNAB bundle\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Delete |bundle| -- it is already in the acronym |CNAB|. %>');
	text = text.replace(/(\bCNAB Bundle\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Delete |Bundle| -- it is already in the acronym |CNAB|. %>');
	...
	text = text.replace(/(\bcollocates\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |co-locates| is now preferred. %>');
	text = text.replace(/(\bcollocating\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |co-locating| is preferred. %>');
	text = text.replace(/(\bCollocating\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Co-locating| is preferred. %>');
	...
	text = text.replace(/(\bcolocates\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |co-locates| is now preferred. %>');
	...
	text = text.replace(/(\bcontainer registry\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# If generic, |container image registry| on first use. If VMware-provided, |Tanzu Network Registry| on first use. In both cases, |registry| thereafter except where risking ambiguity. %>');
	...
	text = text.replace(/(\bCredhub\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The CF component is named |CredHub| %>');
	text = text.replace(/(\bcURL\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |cURL| is the project. |curl| is the CLI. %>');
	...
	text = text.replace(/(\bdedicated VM plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |dedicated-VM plan| is preferred. %>');
	text = text.replace(/(\bDedicated VM plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |dedicated-VM plan| is preferred. %>');
	text = text.replace(/(\bDedicated VM Plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |dedicated-VM plan| is preferred. %>');
	text = text.replace(/(\bdedicated VM Plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |dedicated-VM plan| is preferred. %>');
	...
	text = text.replace(/(\bDetails on\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Details about| is preferred. %>');
	text = text.replace(/(\bdetermine\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Should this actually be |discover| or |learn|? %>');
	text = text.replace(/(\bDetermine\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Should this actually be |discover| or |learn|? %>');
	text = text.replace(/(\bdevelopment environment\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# If this simply refers to the user machine, |local machine| is preferred. If drawing a distinction from a production environment, leave as is. %>');
	...
	text = text.replace(/(\bdisc\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |disk| is preferred. %>');
	...
	text = text.replace(/(\bdo manual\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |manually| might be better. %>');
	text = text.replace(/(\bDo manual\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Manually might be better. %>');
	...
	text = text.replace(/(\bdown time\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |downtime| is preferred. %>');
	...
	text = text.replace(/(\bdropdown menus\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |dropdowns| is preferred. %>');
	text = text.replace(/(\bDropdown menus\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Dropdowns| is preferred. %>');
	text = text.replace(/(\bdue to the fact that\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |because| might be better. %>');
	...
	text = text.replace(/(\beighty\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	text = text.replace(/(\beligible\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Might the plainer |able| be better? %>');
	...
	text = text.replace(/(\bEveryone's\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bexpected\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider replacing with |in most cases| to sound more confident. %>');
	...
	text = text.replace(/(\bFeb\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |February| is preferred. %>');
	text = text.replace(/(\bfifty\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	...
	text = text.replace(/(\bfollowing command\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Replace with just |run:| %>');
	...
	text = text.replace(/(\bfor the sake of\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |to| might be better here. %>');
	text = text.replace(/(\bFor the sake of\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |To| might be better here. %>');
	text = text.replace(/(\bforty\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	...
	text = text.replace(/(\bGlobal Support Services\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |VMware Tanzu Support| is preferred. %>');
	...
	text = text.replace(/(\bGSS\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |VMware Tanzu Support| is preferred. %>');
	...
	text = text.replace(/(\bguid\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |GUID| is preferred. %>');
	...
	text = text.replace(/(\bHaproxy\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |HAProxy| is preferred. %>');
	text = text.replace(/(\bhas been\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider changing to |is| or |has| or rewrite for active voice. %>');
	...
	text = text.replace(/(\bhelm\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Helm| is preferred. %>');
	text = text.replace(/(^[^\n\W]+[^\n#\<\d\(]*Helm Chart\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Helm chart| is preferred. %>');
	...
	text = text.replace(/(\bHuman readable text\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The compound adjective is missing a hyphen. %>');
	text = text.replace(/(\bHyperloglog\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |HyperLogLog| is preferred. %>');
	text = text.replace(/(\bhyperloglog\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |HyperLogLog| is preferred. %>');
	text = text.replace(/(\bHyperloglogs\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |HyperLogLogs| is preferred. %>');
	text = text.replace(/(\bhyperloglogs\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |HyperLogLogs| is preferred. %>');
	...
	text = text.replace(/(\bIDP\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |IdP (identity provider)| is preferred. %>');
	text = text.replace(/(\bIDPs\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |identity providers| is preferred. A single identity provider can be shortened to |IdP|. %>');
	text = text.replace(/(\bIf you want to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Maybe replace with just |To|. %>');
	text = text.replace(/(\bif you want to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Maybe replace with just |to|. %>');
	text = text.replace(/(\bimage registry\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# If generic, |container image registry| on first use. If VMware-provided, |Tanzu Network Registry| on first use. In both cases, |registry| thereafter except where risking ambiguity. %>');
	text = text.replace(/(\bin a [a-z]* manner\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider replacing with an adverb. %>');
	...
	text = text.replace(/(\bIn order to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |To| is preferred. %>');
	text = text.replace(/(\bin preparation for\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |to prepare for| might be better. Verbs are punchier than abstract nouns. %>');
	...
	text = text.replace(/(\bineligible\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Might the plainer |unable| be better? %>');
	...
	text = text.replace(/(\bInternet Protocol\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |IP address| is preferred. %>');
	text = text.replace(/(\bis necessary to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider deleting this or replacing it with the shorter |you must|. %>');
	text = text.replace(/(\bis possible to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |you can| might be better. %>');
	text = text.replace(/(\bis used to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	...
	text = text.replace(/(\bkubernetes\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Kubernetes| is preferred. %>');
	...
	text = text.replace(/(\blog line\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |log entry| is preferred. %>');
	text = text.replace(/(\bLog line\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Log entry| is preferred. %>');
	text = text.replace(/(\blog lines\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |log entries| is preferred. %>');
	text = text.replace(/(\bLog lines\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Log entries| is preferred. %>');
	text = text.replace(/(\blog message\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |log entry| is preferred. %>');
	text = text.replace(/(\bLog message\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Log entry| is preferred. %>');
	text = text.replace(/(\blog messages\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |log entries| is preferred. %>');
	text = text.replace(/(\bLog messages\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Log entries| is preferred. %>');
	...
	text = text.replace(/(\bloggregator(?!\-)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Call it |Loggregator| -- capitalized and without a preceding |the|. %>');
	...
	text = text.replace(/(\bMar\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |March| is preferred. %>');
	...
	text = text.replace(/(\bMax\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Maximum| is preferred. %>');
	text = text.replace(/(\bmax\.(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |maximum| is preferred. %>');
	text = text.replace(/(\bMax\.(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Maximum| is preferred. %>');
	...
	text = text.replace(/(\bMin\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Minimum| is preferred. %>');
	text = text.replace(/(\bmin\.(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |minimum| is preferred. %>');
	text = text.replace(/(\bMin\.(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Minimum| is preferred. %>');
	...
	text = text.replace(/(\bNGINX\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |NGINX| for the brand. |nginx| for files and code. %>');
	text = text.replace(/(\bNginx\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |NGINX| for the brand. |nginx| for files and code. %>');
	text = text.replace(/(\bninety\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	text = text.replace(/(\bnot recommend\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |VMware discourages| is preferred for closed source. |Cloud Foundry discourages| for open source. %>');
	text = text.replace(/(\bnot recommended\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |VMware discourages| is preferred for closed source. |Cloud Foundry discourages| for open source. %>');
	...
	text = text.replace(/(\bnote that\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Notes must be in Note boxes and start with |Note: |. %>');
	text = text.replace(/(\bNov\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |November| is preferred. %>');
	text = text.replace(/(\bnow should\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Favour certainty and agency: |the app now works| over |the app should now work|, and |VMware recommends| over |you should|.  %>');
	...
	text = text.replace(/(\bOauth\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |OAuth| is preferred. %>');
	text = text.replace(/(\bOct\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |October| is preferred. %>');
	text = text.replace(/(\bODB(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_abbr %>');
	text = text.replace(/(\bof your choosing\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |you choose| is snappier and easier for machine translation. %>');
	text = text.replace(/(\bon demand broker(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn demand broker(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn Demand Broker(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn Demand broker(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bon demand plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |on-demand plan| is preferred. %>');
	text = text.replace(/(\bOn demand plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |on-demand plan| is preferred. %>');
	text = text.replace(/(\bOn Demand Plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |on-demand plan| is preferred. %>');
	text = text.replace(/(\bOn Demand plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |on-demand plan| is preferred. %>');
	...
	text = text.replace(/(\bOn-Demand Broker(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/(\bOn-demand Broker(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The full product title is |on-demand service broker|, the short version is |on-demand broker| and the initialism is <%= vars.product_abbr %>');
	text = text.replace(/([\w][\n\s]{0,1}On-demand(?!(\n|\s)(Broker|broker))(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |On-Demand| is preferred -- capitalize the |D| if you capitalized the |O|. %>');
	...
	text = text.replace(/(\bOptionally\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Our style is (Optional) INSTRUCTION -- as a procedure header or a step. %>');
	text = text.replace(/(\borg manager\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Org Manager| is preferred. %>');
	...
	text = text.replace(/(\bPivotal Container Service(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.k8s_runtime_full %>');
	text = text.replace(/(\bPivotal Labs and Services\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The brand name is |VMware Pivotal Labs|. %>');
	text = text.replace(/(\bPivotal Labs and Strategic Services\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The brand name is |VMware Pivotal Labs|. %>');
	text = text.replace(/(\bPivotal Network(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_network %>');
	text = text.replace(/(\bPivotal Services\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The brand name is |VMware Pivotal Labs|. %>');
	text = text.replace(/(\bPivotal Services Marketplace\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The brand name is |Services Marketplace|. %>');
	text = text.replace(/(\bPivotal Software\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Replace with |VMware|. %>');
	text = text.replace(/(\bPivotal, Inc\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Replace with |VMware|. %>');
	...
	text = text.replace(/(\bpod\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Pod| is capitalized per the K8s docs style. %>');
	text = text.replace(/(\bpods\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Pods| is capitalized per the K8s docs style. %>');
	...
	text = text.replace(/(\bpostgresSQL\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |PostgreSQL| is preferred. %>');
	...
	text = text.replace(/(\bQuorum Queue\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |quorum queue| is preferred. %>');
	text = text.replace(/(\bQuorum Queues\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |quorum queues| is preferred. %>');
	text = text.replace(/(\bRabbit MQ\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RabbitMQ| is preferred. %>');
	text = text.replace(/(\bRabbitMQ dashboard\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RabbitMQ Management UI| is preferred. %>');
	text = text.replace(/(\bRabbitMQ Dashboard\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RabbitMQ Management UI| is preferred. %>');
	text = text.replace(/(\bRabbitMQ Management Interface\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RabbitMQ Management UI| is preferred. %>');
	...
	text = text.replace(/(\bram\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RAM| is preferred. %>');
	...
	text = text.replace(/(\bRe-stage\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Restage| is preferred. %>');
	text = text.replace(/(\bre-subscribe\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |resubscribe| is preferred. %>');
	text = text.replace(/(\bRe-subscribe\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Re-subscribe| is preferred. %>');
	...
	text = text.replace(/(\brecommend against\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |VMware discourages| is preferred for closed source. |Cloud Foundry discourages| for open source. %>');
	text = text.replace(/(\brecommends against\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |VMware discourages| is preferred for closed source. |Cloud Foundry discourages| for open source. %>');
	text = text.replace(/(\brecreate\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |re-create| is preferred. %>');
	text = text.replace(/(\bRecreate(?! [A-Z])(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |re-create| is preferred. %>');
	...
	text = text.replace(/(\bRMQ dashboard\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RabbitMQ Management UI| is preferred. %>');
	text = text.replace(/(\bRMQ Dashboard\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RabbitMQ Management UI| is preferred. %>');
	text = text.replace(/(\bRMQ Management Interface\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |RabbitMQ Management UI| is preferred. %>');
	text = text.replace(/(\brun time\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |runtime| is preferred unless referring to the time it takes a program to run. %>');
	text = text.replace(/(\bRun:(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Write what this command does. As in |Do x by running:| or |To do x, run:|. %>');
	...
	text = text.replace(/(\bruntimes\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |runtime durations| or |runtime environments| is preferred, depending on meaning. %>');
	...
	text = text.replace(/(\bService Gateway Access\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The RabbitMQ feature is |Service-gateway access|. %>');
	...
	text = text.replace(/(\bset to off\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |disabled| might be better. %>');
	text = text.replace(/(\bset to on\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |enabled| might be better. %>');
	text = text.replace(/(\bsettings data\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |configuration data| is preferred. %>');
	text = text.replace(/(\bseventy\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	text = text.replace(/(\bsha\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |SHA| is preferred. %>');
	text = text.replace(/(\bshan't\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bshared VM plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |shared-VM plan| is preferred. %>');
	text = text.replace(/(\bShared VM plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |shared-VM plan| is preferred. %>');
	text = text.replace(/(\bShared VM Plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |shared-VM plan| is preferred. %>');
	text = text.replace(/(\bshared VM Plan\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |shared-VM plan| is preferred. %>');
	...
	text = text.replace(/(\bShe's\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bshould now\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Favour certainty and agency: |the app now works| over |the app should now work|. |VMware recommends| over |you should|.  %>');
	...
	text = text.replace(/(\bsince\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use |since| where you can use |because|. %>');
	text = text.replace(/(\bSince\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use |Since| where you can use |Because|. %>');
	text = text.replace(/(\bsixty\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	text = text.replace(/(\bslave\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |follower| is preferred. %>');
	text = text.replace(/(\bsmoketest\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |smoke test| (n.) or |smoke-test| (adj.) is preferred. %>');
	text = text.replace(/(\bSmoketest\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |smoke tests| (n.) or |smoke-tests| (adj.) is preferred. %>');
	text = text.replace(/(\bsmoketests\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |smoke tests| (n.) or |smoke-tests| (adj.) is preferred. %>');
	text = text.replace(/(\bSmoketests\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |smoke tests| (n.) or |smoke-tests| (adj.) is preferred. %>');
	...
	text = text.replace(/(\bSpringBoot\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Spring Boot| is preferred. %>');
	text = text.replace(/(\bssh\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |SSH| is preferred. %>');
	...
	text = text.replace(/(\bstatefulset\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |StatefulSet| is the K8s docs style. %>');
	text = text.replace(/(\bStatefulset\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |StatefulSet| is the K8s docs style. %>');
	text = text.replace(/(\bstatefulsets\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |StatefulSet| is the K8s docs style. %>');
	text = text.replace(/(\bStatefulsets\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |StatefulSet| is the K8s docs style. %>');
	...
	text = text.replace(/(\bstem-cell\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |stemcell| is preferred. %>');
	text = text.replace(/(^[^\n\W]+[^\n#\<\d\(]*Stemcell\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Lowercase is preferred unless |stemcell| starts a sentence or is in a title. %>');
	text = text.replace(/(\bStemCell\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |stemcell| is preferred. %>');
	text = text.replace(/(\bstorageClass\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |StorageClass| is the K8s docs style. %>');
	...
	text = text.replace(/(\bsuper user\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |superuser| is preferred. %>');
	text = text.replace(/(\bSuper user\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Superuser| is preferred. %>');
	...
	text = text.replace(/(\btext area\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |field| is preferred. %>');
	text = text.replace(/(\btextbox\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |field| is preferred. %>');
	text = text.replace(/(\bthat are in effect\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	text = text.replace(/(\bthat is in effect\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	...
	text = text.replace(/(\bThe deploy\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |The deployment| is preferred. %>');
	text = text.replace(/(\bthe following commands\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |these commands| is preferred %>');
	...
	text = text.replace(/(\bThey've\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bthirty\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	text = text.replace(/(\bthrice\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |three times| is preferred. %>');
	text = text.replace(/(\bthus\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Re-write the sentence to drop |thus| or, if that is not possible, replace with |therefore|. %>');
	text = text.replace(/(\bThus\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Re-write the sentence to drop |Thus| or, if that is not possible, replace with |therefore|. %>');
	text = text.replace(/(\bTick\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# For checkboxes, |Enable| or |Select| is preferred. %>');
	text = text.replace(/(\btime out\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |timeout| is preferred, both as a noun and a verb. %>');
	...
	text = text.replace(/(\bTurn off\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |record| is preferred. %>');
	text = text.replace(/(\bturn off\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |disable| is preferred. %>');
	text = text.replace(/(\bTurn on\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |record| is preferred. %>');
	text = text.replace(/(\bturn on\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |enable| is preferred. %>');
	text = text.replace(/(\btwenty\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use numerals for numbers greater than nine. %>');
	...
	text = text.replace(/(\bUAA Client\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |UAA Command Line Client (UAAC)| is preferred for use. |UAAC| thereafter. %>');
	text = text.replace(/(\bUAA Command Client\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |UAA Command Line Client (UAAC)| is preferred for use. |UAAC| thereafter. %>');
	text = text.replace(/(\bUAA command line\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |UAA Command Line Client (UAAC)| is preferred for use. |UAAC| thereafter. %>');
	...
	text = text.replace(/(\bURl\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |URL| is preferred. %>');
	text = text.replace(/(\burl\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |URL| is preferred. %>');
	text = text.replace(/(\bus\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc). %>');
	...
	text = text.replace(/(\buuid\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |UUID| is preferred. %>');
	text = text.replace(/(\bValidate that\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Verify that| is plainer English. %>');
	text = text.replace(/(\bvalidate that\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |verify that| is plainer English. %>');
	...
	text = text.replace(/(\bvm\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |VM| is preferred. %>');
	text = text.replace(/(\bVMT network(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_network %>');
	text = text.replace(/(\bVMT Network(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_network %>');
	text = text.replace(/(\bVMTN(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_network %>');
	...
	text = text.replace(/(\bVMware Tanzu Network(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_network %>');
	text = text.replace(/(\bVMware Tanzu network(?![^\(]*\) %\>)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Use <%= vars.product_network %>');
	text = text.replace(/(\bVoluminous\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |Very long| and |Very large| are plainer alternatives. %>');
	text = text.replace(/(\bvoluminous\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |very long| and |very large| are plainer alternatives. %>');
	...
	text = text.replace(/(\bwe\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc). %>');
	text = text.replace(/(\bWe\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc). %>');
	text = text.replace(/(\bwe'd\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bWe'd\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bwe'll\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bWe'll\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bwe're\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bWe're\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bwe've\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bWe've\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Specify the party (VMware, Cloud Foundry, etc) and do not use contractions. %>');
	text = text.replace(/(\bweb site\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |website| is preferred. %>');
	text = text.replace(/(\bwebdav\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |WebDAV| is preferred. %>');
	text = text.replace(/(\bWebDav\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |WebDAV| is preferred. %>');
	text = text.replace(/(\bWebSockets\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |WebSocket| is preferred: there is a WebSocket protocol, not multiple WebSockets. %>');
	...
	text = text.replace(/(\bWhat's\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not use contractions. %>');
	text = text.replace(/(\bwhen satisfied\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	text = text.replace(/(\bWhen satisfied\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	text = text.replace(/(\bwhen you are satisfied\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	text = text.replace(/(\bWhen you are satisfied\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Redundant? %>');
	...
	text = text.replace(/(\bWire\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |sync| is preferred. %>');
	text = text.replace(/(\bwire\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |sync| is preferred. %>');
	text = text.replace(/(\bWiring\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |sync| is preferred. %>');
	text = text.replace(/(\bwiring\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# |sync| is preferred. %>');
	...
	text = text.replace(/(\byou choose to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider if |choose| is redundant. %>');
	text = text.replace(/(\byou like\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Translation engines might struggle with this. %>');
	text = text.replace(/(\byou might want\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# If possible, replace with |VMware recommends| (or |Cloud Foundry recommends| if open source) or re-phrase as a mandate. %>');
	text = text.replace(/(\byou need to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider replacing with just |To|. %>');
	text = text.replace(/(\bYou need to\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Consider replacing with just |To|. %>');
	...
	text = text.replace(/(\bYoutube\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# The brand name is |YouTube|. %>');
	text = text.replace(/([^u][^b][\n\s]Maestro\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Do not shorten |CredHub Maestro| to just |Maestro| %>');
	text = text.replace(/(\s'(?![^\[\(]*[\]\)])(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Should this be a backtick? %>');
	text = text.replace(/('[^\]\)\w](?![^\[\(]*[\]\)])(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Should this be a backtick? %>');
	text = text.replace(/(\btime to live\b(?!\s\()(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Shorten to |TTL| after first use. %>');
	text = text.replace(/(\bsteps[\s\n]*\w*:(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# To introduce steps just write |To do x:| %>');
	text = text.replace(/(\bsteps[\s\n]*\w*:(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# To introduce steps just write |To do x:| %>');
	text = text.replace(/([^f] the following:(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# To introduce steps just write |To do x:| %>');
	...
	text = text.replace(/(\bWhile\b(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# Replace with |Although| if it makes sense to do so. %>');
	text = text.replace(/((fixes|issues|this release):\n\n\*\s[^\*](?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# In release notes, each feature, KI, and resolved issue needs a run-in heading. %>');
	text = text.replace(/(^\*\s[^\*][^#]*###\s(Resolved Issues|Known Issues|Features)(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# In release notes, each feature, KI, and resolved issue needs a run-in heading. %>');
	text = text.replace(/(^##+[^\n]*\n+[^\n]*[^:]\n\n+(?=\s*(\<ol\>|\d\.\s))(?![^£]*\<%# ECS %\>)(?![^\{]*\}))/gm,'$1<%# A procedure requires a stem ending with a colon, as in |To do x:|. %>');

	// Delete delimiters for code formatting tags

	text = text.replace(/(\<%# £SCS %\>)/gm,'');
	text = text.replace(/(\<%# ECS %\>)/gm,'');

	// Delete comments about comments
