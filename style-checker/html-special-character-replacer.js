...
text = text.replace(/(\b2012R2\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# The product is called "Pivotal Application Service for Windows 2012 R2" or, for short, "PASW 2012 R2" %>');
...
text = text.replace(/(\brefer\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# "see" is preferred. %>');
...
text = text.replace(/(\bSecurity Assertion Markup Language\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# "SAML" is preferred. %>');
...
text = text.replace(/(\bSomeone's\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# We do not use contractions. %>');
text = text.replace(/(\bSpace\b(?![^£]*\<%# ECS %\>))/gm,'$1<%# Only capitalize "space" when it is part of a title %>');
