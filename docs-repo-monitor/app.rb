...
@push = JSON.parse(request.body.read) # push data0
@array_of_commits = []
@repo = ""
@new_branch = ""
...
def releasenotes
  ...
  pre_release_hash = client.refs(@repo)
  pre_release_hash.each do |i|
    if i['ref'] == "refs/heads/" + '1.6'
      @pre_release_sha = i['object']['sha']
    end
  end
  

  @new_branch = "refs/heads/master-to-1.6-" + @pre_release_sha[0..5]
  ...
  if @conflicts.length > 0 
    client.create_issue(@repo, "Master to " + '1.6', @issue_message + @conflicts.to_s)
    print "Issue created."
    client.delete_ref(@repo, "heads/master-to-1.6-" + @pre_release_sha[0..5])
    print "Branch deleted."
  else
    @merge_msg = "Merge this pull request to bring commits into the 1.6 branch."
    client.create_pull_request(@repo, "refs/heads/" + '1.6', @new_branch, "Master to " + '1.6', @merge_msg)
    print "PR created."
  end
end